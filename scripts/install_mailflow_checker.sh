...
#
# Options/Env:
#   MAILFLOW_REPO_URL   Git repository URL (default: current repo when running locally; otherwise must be provided)
#   MAILFLOW_BRANCH     Git branch to checkout (default: main)
#   MAILFLOW_INTERVAL   systemd timer interval (default: 5m), examples: 1m, 2m, 10m
#   MAILFLOW_USER       System user to run the service (default: mailflowchecker)
#
...
APP_USER="${MAILFLOW_USER:-mailflowchecker}"
...
REPO_URL="${MAILFLOW_REPO_URL:-https://github.com/fabianpetri/mailflow-checker.git}"
BRANCH="${MAILFLOW_BRANCH:-main}"
INTERVAL="${MAILFLOW_INTERVAL:-5m}"
  ...
  if [[ -d "${APP_DIR}/.git" ]]; then
    ...
    if [[ -z "${REPO_URL}" ]]; then
      ...
      if [[ -d .git ]]; then
        ...
        log_err "MAILFLOW_REPO_URL not provided and current dir is not a git repo. Set MAILFLOW_REPO_URL to your GitHub repo."
        exit 1
      fi
    else
...
Environment variables:
  MAILFLOW_REPO_URL  Git repo URL (e.g. https://github.com/OWNER/REPO.git)
  MAILFLOW_BRANCH    Branch name (default: main)
  MAILFLOW_INTERVAL  Timer interval (default: 5m)
  MAILFLOW_USER      System user (default: mailflowchecker)
USAGE
}
